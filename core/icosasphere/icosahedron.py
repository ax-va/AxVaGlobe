...
class Icosahedron:
    ...
    def index_np_array(self):
        return np.array([
            [0, 1, 2],  # vertex indices [index0, index1, index2] of triangle 0
            [0, 2, 3],  # triangle 1
            [0, 3, 4],  # triangle 2
            [0, 4, 5],  # triangle 3
            [0, 5, 1],  # triangle 4
...
if __name__ == "__main__":
    ics = Icosahedron()
    print(ics.vertex_np_array)
    print(ics.index_np_array)
    print(ics.vertex_np_array.dtype)
    print(ics.index_np_array.dtype)
    print(ics.vertex_np_array.shape)
    print(ics.index_np_array.shape)
    # print(np.array([1, 2, 3]).shape)
    # print(np.array([[1, 2, 3], [4, 5, 6]]))
    # print(np.random.random(3))
    a = np.array([[1, 2], [3, 4]])
    print(a.astype(dtype=np.float32).flatten(order='F'))
