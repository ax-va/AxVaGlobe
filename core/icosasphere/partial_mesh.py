...
class PartialMesh(AnyMesh):
    ...
    def add_nodes(self, nodes):
        for node in nodes:
            if node.index not in self._vertex_dict:
                self._add_node(node)
            for neighbor in node.neighboring_nodes:
                if neighbor.index not in self._vertex_dict:
                    self._add_node(neighbor)
            for triangle in node.adjacent_triangles:
                if (triangle.index + self._index_offset) not in self._vertex_dict:
                    ...

    def _add_edge_node(self, edge, node):
        node0, node1 = edge.icosahedron_nodes
        np_vertex0 = self._vertex_dict[node0.index]
        np_vertex1 = self._vertex_dict[node1.index]
        radians = self._theta_factor * node0.division_ratios.ratio0
        self._vertex_dict[node.index] = get_rotated_np_vertex(np_vertex0, np_vertex1, radians)
    ...
    def _add_icosahedron_nodes(self):
        ...
        self._vertex_dict = dict(zip(icosahedron_nodes_indices, icosahedron_vertices))
    ...
    def _add_non_edge_node(self, node):
        ...
        np_vertex0 = self._vertex_cash[node0.index]
        np_vertex1 = self._vertex_cash[node1.index]
