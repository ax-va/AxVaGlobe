...
@pytest.mark.parametrize(
    "index,node_index_offset,number_of_nodes",
    [
        (2, 6, 10),
    ],
)
def test_node_layer_ab_for_partition_two(
    ...
    assert node_layer_ab.NODE_INDEX_OFFSET == node_index_offset
...
def test_node_layer_ab_for_partition_three(
    ...
    assert node_layer_ab.NODE_INDEX_OFFSET == node_index_offset
    assert node_layer_ab.NUMBER_OF_NODES == number_of_nodes
    assert node_layer_ab.LAST_IN_LAYER_INDEX == last_in_layer_index
