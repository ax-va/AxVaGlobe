from axvaglobe.core.schema._base_node_layer import _BaseNodeLayer


class _NodeLayerC(_BaseNodeLayer):
    """
    The class is private because the constructor parameters are not validated.
    Instances of the class must only be created through the factory method, which performs validation.
    """

    @property
    def NODE_INDEX_OFFSET(self) -> int:
        if self._node_index_offset is None:
            end_index: int = self._partition_obj.south_pole.node_layer.INDEX
            end_index_for_area_c: int = self._partition_obj.area_c.node_layers.END
            inverse_index_offset_for_area_c = end_index - end_index_for_area_c
            inverse_index = end_index - self.INDEX
            inverse_sum_of_previous_layer_indices = (
                inverse_index * (inverse_index - 1) // 2
            )
            inverse_node_index_offset = (
                inverse_sum_of_previous_layer_indices * 5
                + inverse_index_offset_for_area_c
            )
            end_node_index: int = self._partition_obj.south_pole.node.INDEX
            self._node_index_offset = (
                end_node_index
                - inverse_node_index_offset
                - (self.NUMBER_OF_NODES - 1)
            )

        return self._node_index_offset

    @property
    def NUMBER_OF_NODES(self) -> int:
        if self._number_of_nodes is None:
            end_index: int = self._partition_obj.south_pole.node_layer.INDEX
            inverse_index = end_index - self.INDEX
            self._number_of_nodes = inverse_index * 5
        return self._number_of_nodes
