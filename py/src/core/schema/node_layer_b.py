...
class _NodeLayerB(BaseNodeLayer):
    @property
    def NODE_INDEX_OFFSET_FOR_LAYER(self) -> int:
        if self._node_index_offset_for_layer is None:
            ...
            relative_layer_index: int = self.INDEX - self._schema.constants.area_a.node_layers.NUMBER - 2
    ...
    def NUMBER_OF_NODES(self) -> int:
        if self._number_of_nodes is None:
            self._number_of_nodes: int = self._schema.constants.border_ab.nodes.NUMBER
